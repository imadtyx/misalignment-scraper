...
_GROK_RE = re.compile(r"^https?://(grok|xai)\.(com|ai)/share/", re.IGNORECASE)


def _is_share_url(url: str) -> bool:
    return bool(_GROK_RE.match(url.strip()))
...
def _extract_from_dom(page: Page) -> List[Dict[str, Any]]:
    ...
    for cl in clusters:
        ...
        if any(k in rattr for k in ("assistant", "model", "grok")):
            role = "model"
...
class GrokScraper:
    ...
    def scrape(self, share_url: str) -> Transcript:
        if not _is_share_url(share_url):
            raise ValueError("Provide a valid Grok share URL")
